...
for i in range(10):
    ...
    no_axis_show(plt.imread(f'real_or_drawing/train_data/{i}/{500 * i}.bmp'), title=titles[i])
plt.show()

plt.figure(figsize=(18, 18))
for i in range(10):
    plt.subplot(1, 10, i+1)
    no_axis_show(plt.imread(f'real_or_drawing/test_data/0/' + str(i).rjust(5, '0') + '.bmp'))
plt.show()

original_img = plt.imread(f'real_or_drawing/train_data/1/510.bmp')
plt.subplot(1, 5, 1)
